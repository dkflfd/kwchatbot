...
class kwchatbotError(Exception):
    pass
...
class SentenceFactory(object):
    ...
    def text2sentences(self, text):
        ...
        for candidate in candidates:
            #구분자로 ?와 ! 추가
            while len(candidate) and (candidate[-1] == '.' or candidate[-1] == ' ' or candidate[-1] == '?' or candidate[-1] == '!'):
...
class kwchatbot(object):

    def __init__(self, similarity='cosine', tagger='twitter', useful_tags=['Noun', 'Verb', 'Adjective', 'Determiner', 'Adverb', 'Conjunction', 'Josa', 'PreEomi', 'Eomi', 'Suffix', 'Alpha', 'Number'],
                 delimiters=['. ', '\n', '.\n', '? ', '! '], stopwords=stopwords_ko, max_dictionary_size=None, min_cluster_size=2, 
                 similarity_threshold=0.85, matrix_smoothing=False, **kwargs):
    ...
    def _build_matrix(self):
        self.matrix = np.zeros((self.num_sentences, 1))        
        sentence2 = self.sentences[0]
        for sentence1 in self.sentences:
            self.matrix[sentence1.index, 0] = self.sim(sentence1, sentence2)
            if(self.matrix[sentence1.index, 0] == 1) and (sentence1.index != 0) :
                break
